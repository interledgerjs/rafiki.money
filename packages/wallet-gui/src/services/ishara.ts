...
const ISHARA_API_URL = process.env.REACT_APP_WALLET_API_URL || 'http://localhost:3000/api/'

export const ishara = {
  getMandates: async (user: string, state?: string) => {
    const url = new URL('mandates', ISHARA_API_URL)
    return axios.get(url.toString() + `?userId=${user}` + (state ? `&state=${state}` : '')).then(response => response.data)
  },
  ...
  getConsent: async (challenge: string) => {
    ...
    return axios.get(url.toString() + `?consent_challenge=${challenge}`).then(response => response.data)
  },
