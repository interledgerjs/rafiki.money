...
const Login: NextPage<Props> = ({ login_challenge }) => {
  ...
  const onSubmit = async data => {
    if (validateEmail({ target: { value: data.username } })) {
      const login = await usersService
        .login(data.username, data.password, login_challenge)
        .then(resp => {
          if (resp.redirectTo) {
        ...
        .catch(async error => {
          ...
          body.errors.forEach(el => {
            if (el.field === 'username') setError('username', 'usernameError', el.message)
            else if (el.field === 'password') setError('password', 'passwordError', el.message)
          })
        })
    }
  }
      ...
      <div className="w-full h-screen max-w-xs mx-auto bg-surface flex items-center">
        <form ref={formRef} className="w-full max-w-xs" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="text-center my-12">
            ...
            <Button disabled={Object.keys(errors).length > 0} bgColour="primary" type="solid" buttonType="submit">
              LOGIN
            </Button>
          </div>
        </form>
...
Login.getInitialProps = async ({ query, res }) => {
  ...
  const login = await usersService
    .getLogin(login_challenge.toString())
    .then(resp => {
      if (resp.redirectTo) {
        res.writeHead(302, {
