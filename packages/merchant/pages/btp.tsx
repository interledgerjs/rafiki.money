...
import { IlpPrepare, serializeIlpPrepare, deserializeIlpPacket } from "ilp-packet"
...
const Page: NextPage = () => {
  ...
  const [url, setUrl] = useState<string>("https://ishara.rafikilabs.com/ilp")
  const [token, setToken] = useState<string>("RKrt7pj1KnyMqSPo2wMVd9k-rW34ItepAAybEO7lU2E.z54coI2nm1uF9jssXbSgBDKIu_3p-KZhGl88M6qCBI0")
  ...
  useEffect(() => {
    const btp = new AbstractBtpPlugin({
      server: "btp+ws://:blahblahblah@localhost:6868"
    })
    btp.connect().catch(error => {
      console.log(error)
    })
  }, )
